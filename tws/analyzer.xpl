   ...
   Blanks between tokens are not significant.
                                                                       */
/* 11/16/2017 Modified to support more than 255 tokens.
...
/* Define the number of bits in a production number */
declare PRO_BITS literally '8',
   PRO_SIZE literally '256',
   PRO_MASK literally '(PRO_SIZE-1)',
   HEAD_SIZE literally 'PRO_SIZE*PRO_SIZE/8-1',
   WORK_SIZE literally 'PRO_SIZE*PRO_SIZE/4-1';
 
   declare TRUE literally '1', FALSE literally '0';
   declare v(PRO_MASK) character,
      (left_part, right_head) (PRO_MASK) bit(PRO_BITS),
      production(PRO_MASK) bit(PRO_BITS * 4),
      (on_left, on_right) (PRO_MASK) bit(1);
   declare pv bit(PRO_BITS * 4);   /* Temp storage for productions */
   declare index(PRO_MASK) bit(PRO_BITS), (ind, ind1) (PRO_MASK) bit(16);
   declare sort#(PRO_MASK) bit(PRO_BITS * 4), ambiguous bit(1);
   declare (nsy, npr, sp, cp, nt, level, goal_symbol) fixed;
 
   declare change bit(1), work(WORK_SIZE) bit(8);
   declare head(PRO_MASK) bit(PRO_BITS * 4), tail(PRO_MASK) bit(PRO_BITS);
   declare STACKLIMIT literally '200', TEXTLIMIT literally '255';
   /*  The minimum value for DEPTH is the number of productions  */
   declare DEPTH literally 'PRO_MASK';
   declare stack(STACKLIMIT) bit(PRO_BITS), text(TEXTLIMIT) bit(PRO_BITS),
      token_save(DEPTH) bit(PRO_BITS), token fixed,
      mp_save(DEPTH) bit(PRO_BITS), mp fixed,
      tp_save(DEPTH) bit(PRO_BITS), tp fixed,
      p_save(DEPTH) bit(PRO_BITS), p fixed;
   declare head_table(HEAD_SIZE) bit(8);
...
declare MAXNF11 literally '23003', MAXNTRIP literally '12000';
   declare f11(MAXNF11) bit(PRO_BITS * 4),  nf11 fixed;
   declare triple(MAXNTRIP) bit(PRO_BITS * 4), tv(MAXNTRIP) bit(2), ntrip fixed;
   declare stacking bit(1) initial (TRUE);  /* controls batching of grammars  */
   declare MAXTROUBLE literally '50', trouble1(MAXTROUBLE) bit(PRO_BITS),
      trouble2(MAXTROUBLE) bit(PRO_BITS), trouble_count fixed;
   ...
   declare PUNCH literally 'punch_out',
      ...
      DOUBLE_SPACE literally 'output(0) = ''''; output(0) = ''''';
...
is_head:
   ...
   declare (i, j) fixed;
   return 1 & shr(head_table(shl(i,PRO_BITS-3)+shr(j,3)), j & 7);
end is_head;
...
get:
   ...
   return 3 & shr(work(shl(i,PRO_BITS-2)+shr(j,2)), shl(j & 3, 1));
end get;
...
pack:
   procedure (b1, b2, b3, b4) bit(PRO_BITS * 4);
   /* This procedure has the value of the 4 bytes packed into a 32-bit word  */
 
   declare (b1, b2, b3, b4) bit(PRO_BITS * 4);
   return shl(b1,PRO_BITS*3) + shl(b2,PRO_BITS*2) + shl(b3,PRO_BITS) + b4;
end pack;
...
enter:
   ...
   declare (val, i, j, k) fixed;
   declare env bit(PRO_BITS * 4);
...
print_triples:
   ...
   declare title character, i fixed;
   declare j bit(PRO_BITS * 4);
   if ntrip = 0 then
      do;
         DOUBLE_SPACE;
   ...
   do i = 1 to ntrip;
      ...
      if k = 3 then
         do;
            ...
            call add_trouble (shr(j, PRO_BITS*2), shr(j, PRO_BITS) & PRO_MASK);
         end;
      call line_out (i, print(k) || ' for  ' || v(shr(j, PRO_BITS*2)) || ' ' ||
         v(shr(j, PRO_BITS)&PRO_MASK) || ' ' || v(j&PRO_MASK));
      count(k) = count(k) + 1;
   end;
   DOUBLE_SPACE;
...
look_up:
   procedure (symbol) bit(PRO_BITS);  /* Get index of symbol in v  */
...
expand:
   ...
   stack(1) = shr(f11, PRO_BITS) & PRO_MASK;  /* left context */
   stack(sp+1) = f11 & PRO_MASK;         /* right context */
end expand;
...
read_grammar:
   ...
   declare long fixed;
   declare prod bit(PRO_BITS * 4);
 
   scan:
      procedure bit(PRO_BITS); /* get a token from input cardimage  */
   ...
   sort_v:
      ...
      do i = 1 to nsy;
         ...
         sort#(i) = shl(on_left(i), PRO_BITS * 2) | shl(length(v(i)), PRO_BITS) | i;
      end;
      /* bubble sort  */
      k, l = nsy;
      do while k <= l;
         ...
         do i = 2 to k;
            ...
            if sort#(l) > sort#(i) then
               do;
                  prod = sort#(l);  sort#(l) = sort#(i);  sort#(i) = prod;
                  t = v(l);  v(l) = v(i);  v(i) = t;
                  k = l;
               end;
         end;
      ...
      do i = 1 to nsy; /* build table to locate sorted symbols of v */
         index(sort#(i)&PRO_MASK) = i;
      end;
      nt = nsy;  /* prepare to count non-terminal symbols */
      do while sort#(nt) > shl(1, PRO_BITS * 2);  nt = nt - 1;  end;
      ...
      do i = 1 to npr;
         ...
         pv = 0;
         do k = 0 to 3;
            j = index(shr(prod, PRO_BITS * 3) & PRO_MASK);
            on_right(j) = TRUE;
            prod = shl(prod, PRO_BITS);
            pv = shl(pv, PRO_BITS) + j;
         end;
         production(i) = pv;
      end;
   ...
   nsy = 1;  v(1) = '_|_';
   npr, error_count = 0;
   do i = 0 to PRO_MASK;  /* clear on_left and on_right  */
      on_left(i), on_right(i) = 0;
   end;
   do while get_card;  /* watch side effect */
      if npr = PRO_MASK then call error ('too many productions');
      else npr = npr + 1;
      ...
      do j = 1 to 4;
         i = scan;
         if i ~= 0 then
            prod = shl(prod, PRO_BITS) + i;  /* pack 4 to a word  */
      end;
      ...
      if control(byte('P')) | control(byte('C')) then
         do;  /* unpack table and print XPL comment */
            ...
            outcard = ' /*  ' || v(left_part(npr)) || ' ::= ' ||
               v(right_head(npr));
            do k = 0 to PRO_BITS * 3 by PRO_BITS;
               i = shr(pv, PRO_BITS * 3 - k) & PRO_MASK;
      ...
      do i = 1 to trouble_count;  /* step through problems  */
         ...
         do p = 1 to basic_npr;
            ...
            do l = 1 to 4;  /* step through right part  */
               s2 = shr(pv, PRO_BITS * 3) & PRO_MASK;
               pv = shl(pv, PRO_BITS);
               newp = shl(newp, PRO_BITS) + s2;
               if s2 ~= 0 then
                  do;
                     if s1 = t1 & is_head (s2, t2) then
                        do;
                           ...
                           if nsy < PRO_MASK then nsy = nsy + 1;
                           else call error ('too many symbols');
                           s = substr(add_on, j, 1);
                           v(nsy) = '<' || v(t1) || s || '>';
                           if npr < PRO_MASK then npr = npr + 1;
                           ...
                           change = TRUE;
                           if internal then do;
                              newp = newp - (newp & shl(PRO_MASK, PRO_BITS));
                              newp = newp | shl(nsy, PRO_BITS);
                           end;
...
compute_heads:
   ...
   if iteration_count = 1 then
      do;
         ambiguous = FALSE;
         do i = 1 to npr;
            j = right_head(i);  pv = production(i);
            k = PRO_BITS * 3;
            do while k >= 0 & pv ~= 0;
               /* I don't understand why this is done in a loop */
               j = shr(pv, k) & PRO_MASK;
               k = k - PRO_BITS;
            end;
...
produce:
   ...
   apply_production:
      procedure;  /* Perform one parse step (on stack and text) and recur  */
         ...
         pv = production(p);
         do j = 0 to PRO_BITS * 3 by PRO_BITS;
            k = shr(pv, PRO_BITS * 3 - j) & PRO_MASK;
            if k ~= 0 then do;
...
index_f11:
   ...
   declare (y, p, i, j, k, m) fixed;
   declare yp bit(PRO_BITS * 4);
   ...
   y, yp = 0;
   f11(nf11 + 1) = shl(nsy+1, PRO_BITS * 2);  /* boundary condition  */
   do i = 1 to nf11+1;  /* check each f11  */
      if f11(i) >= yp then
         do;      /* f11 for a new left part */
            ...
            y = shr(f11(i), PRO_BITS * 2) & PRO_MASK;    /* new left part */
            p = index(y);
            do while left_part(p) = y;    /* record start for new left part */
               ind(p) = i;
               p = p + 1;
            end;
            yp = shl(y + 1, PRO_BITS * 2);   /* to compare with triple */
         end;
   end;
end index_f11;
...
sort_productions:
   ...
   declare hv bit(PRO_BITS * 4);
   do i = 1 to npr;  p_save(i) = 0;  end;
   p = 0;
   do i = 1 to npr;
      hv = right_head(i);  /* convert 1,2,3,4 packing to 4,3,2,1 packing  */
      pv = production(i);
      if pv = 0 then
         do;
            pv = hv;
      ...
      pv = pv - tail(i);
      do while pv ~= 0;
         k = shr(pv, PRO_BITS * 3) & PRO_MASK;  pv = shl(pv, PRO_BITS);
         if k ~= 0 then
            do;
               hv = shl(hv, PRO_BITS) + k;
               l = l - 1;
            end;
      end;
      head(i) = hv;
   ...
   do while k <= l;
      ...
      do i = 2 to k;
         ...
         if sort#(l) > sort#(i) then
            do;
               pv = sort#(l);  sort#(l) = sort#(i);  sort#(i) = pv;
...
compute_c1:
   procedure;
      declare (cx, ctrip, s1, s2, s3) fixed;
      declare (ptrip, ttp) bit(PRO_BITS * 4);
               ...
               do s3 = 1 to nt;     /* step through the heads of stack(j+1)  */
                  if is_head(l, s3) then
                     do case cx;
                        ...
                        do;
                           ptrip = pack (0, s1, s2, s3);
                           do m = 0 to ctrip;
                              if sort#(m) = ptrip then
            ...
            do;
               ...
               do m = 0 to ctrip;  /* step through conflicts  */
                  do k = 0 to 1;  /* step through truth values  */
                     ttp = sort#(m);
                     output = '';
                     output = '   The triple  ' || v(shr(ttp, PRO_BITS * 2) &
                        PRO_MASK) || ' ' || v(shr(ttp, PRO_BITS) & PRO_MASK) ||
                        ' ' || v(ttp & PRO_MASK) || '  must have the value ' ||
                        print(value(k)) || ' for';
                     output = '';
                     ttp = m + 1;
                     ttp = shl(ttp, PRO_BITS * 3);
                     i = j;
                     s1 = 0;
                     do while triple(i) < ttp & i <= ntrip;
                        if (tv(i) & value(k)) ~= 0 then
                           do;
                              ptrip = triple(i);
                              p = shr(ptrip, PRO_BITS * 2) & PRO_MASK;
                              if p ~= s1 then call output_production (p);
                              s1 = p;
                              output = '         in the context  ' ||
                                 v(shr(ptrip, PRO_BITS) & PRO_MASK) || dots ||
                                 v(ptrip & PRO_MASK);
                           end;
                        i = i + 1;
                     end;     /* of do while  */
   ...
   /* Determine what (if any) context must be checked for each production
      ...
      do j = 1 to npr - 1;
         ...
         do while tail(ij) = tail(ik);  /* check all productions with same tail */
            tj = head(ij);
            tk = head(ik);
            do while (tj & PRO_MASK) = (tk & PRO_MASK) & tj ~= 0;
               tj = shr(tj, PRO_BITS);  tk = shr(tk, PRO_BITS);
            end;
            if tk = 0 then
               do;      /* production ik is included in ij  */
                  ...
                  do pj = ind(ij) to ind1(ij);
                     jct = f11(pj) & (shl(PRO_MASK, PRO_BITS) + PRO_MASK);
                     jcr = jct & PRO_MASK;
                     if proper then jct = jcl | jcr;
                     else jcl = jct & shl(PRO_MASK, PRO_BITS);
                     do pk = ind(ik) to ind1(ik);
                        kct = f11(pk) & (shl(PRO_MASK, PRO_BITS) + PRO_MASK);
                        if kct = jct then
                           do;
...
'   They have equal right parts and the common context  '
                                 || v(shr(jcl, PRO_BITS)) || dots || v(jcr);
                              call add_trouble (shr(kct, PRO_BITS), left_part(ik));
                           end;
                        else if (kct & shl(PRO_MASK, PRO_BITS)) = jcl then
                           mp = mp | 1;  /* can't tell by left context */
                        else if (kct & PRO_MASK) = jcr then
...
'         (1,0) context will be checked.  Legal left context:';
               ...
               do pj = ind(ij) to ind1(ij);
                  jcl = shr(f11(pj), PRO_BITS) & PRO_MASK;
                  if ~ on_left(jcl) then
                     do;
                        on_left(jcl) = TRUE;
            ...
            do;
               ...
               do pj = ind(ij) to ind1(ij);
                  output = x12 || v(shr(f11(pj), PRO_BITS) & PRO_MASK) ||
                     dots || v(f11(pj) & PRO_MASK);
               end;
            end;
...
punch_productions:
   ...
   if nt >= 254 then wide = 16; else wide = 8;
   l = length(v(nt));
   call build_card ('declare v_index(' || l || ') bit(' || wide || ') initial (');
   j = 1;
   do i = 1 to l;
      call build_card (j || ',');
   ...
   if k < 0 then
      ...
      do;
         ...
         call build_card ('declare c1triples(NC1TRIPLES) bit(' ||
            PRO_BITS * 4 || ') initial (');
         j = 0;
         do i = 1 to ntrip;
            if tv(i) = 1 then
               do;
  /* HUH this should be fixed  "(1)0(4)34 (1)1(4)42" */
   ...
   call build_card ('declare prtb(' || npr || (') bit(' || PRO_BITS * 4 ||
      ') initial (0,'));
   do i = 1 to npr - 1;
      call build_card (head(index(i)) || ',');
   end;
   call punch_card (head(index(npr)) || ');');
   if npr > 255 then wide = 16; else wide = 8;
   call build_card ('declare prdtb(' || npr || ') bit(' || wide || ') initial (0,');
   do i = 1 to npr;
      l = index(i);
   ...
   if npr > 255 then wide = 16; else wide = 8;
   call build_card ('declare hdtb(' || npr || ') bit(' || wide || ') initial (0,');
   do i = 1 to npr - 1;
   ...
   if npr > 255 then wide = 16; else wide = 8;
   call build_card ('declare left_context(' || j - 1 || ') bit(' ||
      wide || ') initial (');
   do i = 0 to j - 2;
   ...
   do i = nt + 1 to nsy;
      ...
      if shr(tp_save(i), 1) then  /* need both contexts */
         do;
            k = token_save(i);
            do l = ind(k) to ind1(k);
               triple(j) = f11(l) & (shl(PRO_MASK, PRO_BITS) + PRO_MASK);
               j = j + 1;
            end;
         end;
   end;
   if j = 0 then j = 1;  /* assure non-negative upper bound for array */
   call build_card ('declare context_triple(' || j-1 || ') bit(' ||
      PRO_BITS * 4 || ') initial (');
   do i = 0 to j - 2;
   ...
   if npr >= 255 then wide = 16; else wide = 8;
   call build_card ('declare pr_index(' || nsy || ') bit(' || wide ||
      ') initial (');
   do i = 1 to nsy;
...
punch_c_productions:
   ...
   if nt > 255 then call build_card ('short');
   else call build_card ('unsigned char');
   l = length(v(nt));
   call build_card ('v_index[' || l + 1 || '] = {');
   j = 1;
   do i = 1 to l;
      call build_card (j || ',');
      do while length(v(j)) = i;  j = j + 1;  end;
   ...
   if k < 0 then
      ...
      do;
         ...
         if PRO_BITS * 4 > 32 then call build_card ('long');
         else call build_card ('int');
         call build_card ('c1triples[NC1TRIPLES + 1] = {');
         j = 0;
         do i = 1 to ntrip;
            if tv(i) = 1 then
               do;
   ...
   call build_card ('prtb[' || npr + 1 || '] = {0,');
   do i = 1 to npr - 1;
      call build_card (head(index(i)) || ',');
   end;
   call punch_card (head(index(npr)) || '};');
   if npr > 255 then call build_card ('short');
   else call build_card ('unsigned char');
   call build_card ('prdtb[' || npr + 1 || '] = {0,');
   do i = 1 to npr;
      l = index(i);
      if l > basic_npr then l = 0;
      if i < npr then call build_card (l || ',');
   ...
   call build_card ('left_context[' || j || '] = {');
   do i = 0 to j - 2;
      call build_card (work(i) || ',');
   end;
   call punch_card (work(j-1) || '};');
   if j > 255 then call build_card ('short');
   else call build_card ('unsigned char');
   call build_card('left_index[' || nsy - nt + 1 || '] = {');
   do i = nt + 1 to nsy;
      call build_card (sort#(i) || ',');
   end;
   call punch_card (j || '};');
   ...
   if j = 0 then j = 1;  /* assure non-negative upper bound for array */
   call build_card ('context_triple[' || j || '] = {');
   do i = 0 to j - 2;
      call build_card (triple(i) || ',');
   end;
   call punch_card (triple(j-1) || '};');
   ...
   call build_card ('pr_index[' || nsy + 1 || '] = {');
   do i = 1 to nsy;
      call build_card (p_save(i) || ',');
   end;
   call punch_card (npr+1 || '};');
